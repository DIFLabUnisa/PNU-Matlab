function [imgFiltrata]=filtro_PNU(img,sigma0)

% La funzione implementa il filtro PNU
% [imgFiltrata]=filtro_PNU(img,sigma0);
...
for num_Canale=1:nCanali
    ...
    [C,S]=wavedec2(imgCanaleNoise,4,'db8'); % wavedec2(immagine,livelli,famiglia wavelet);
    ...
    %fprintf('Compute PNU_Filter (db8): waiting ... Start function on channel %d\n',num_Canale);
    ...
    for numLivello=1:4
        ...
        tmp1 = (sigma0^2);
        tmp2 = (MinVarianzaCH+tmp1);
        tmp3 = (MinVarianzaCH./tmp2);
        cHden = cH.*tmp3;
    ...
    chan_reconstruct=waverec2(C,S,'db8');
    imgFiltrata(:,:,num_Canale)=chan_reconstruct;
end
