function computeCorrelazioni_resizeAdattivo(directoryImages, ReferencePattern, stringaPattern, percorsoFileOutput, descrizione,db)

% computeCorrelazioni_resizeAdattivo(directoryImages, ReferencePattern, stringaPattern, percorsoFileOutput, descrizione,db)
%
...
% quest'ultimo potrebbe essere ridimensionato (scalato) 
%
% Autori: Lanzilli Giuseppe- Roscigno Gianluca


ReferencePatternOriginale=ReferencePattern;
...
for i = 1 : length(files)
    if ~directory(i).isdir % escludo qualsiasi cartella, anche ./ ../
        ...
        fprintf('Compute Photo %d <<%s>>   [Utilizzo del RESIZE]\n',i-2,char(files(i)));
        img=imread(strcat(directoryImages,char(files(i))));
        [righe, colonne, canali] = size(img);

        t0=clock;
	t=cputime;
        [residual_noise]=getResidualNoiseAdattivo2(img,db);
	tempoCPU = cputime-t; 
