function confrontoDimensioniLimitate(pathReferencePatterns, pathImages, numImg, fileOutput, classe, dimensioni)

% confrontoDimensioniLimitate(pathReferencePatterns, pathImages, numImg, fileOutput, classe, dimensioni)
%
...
fprintf('\nCartella Immagini <<%s>> CLASSE %d  (%dx%d):\n', pathImages, classe, dimensioni(1), dimensioni(2));
...
for j=1:length(files)
	...
	line=sprintf('%d', classe);
       
        % Sto elaborando il file "nomeFile"
        fprintf('Sto elaborando l''immagine <<%s%s>> Classe %d\n', pathImages, nomeFile, classe);
        
        
	immagine1=imread(strcat(pathImages,nomeFile));

	immagine=immagine1(1:dimensioni(1), 1:dimensioni(2), :);

	[righe, colonne, canali] = size(immagine);
        
        %fprintf('OUTPUT FILTRO\n'); 
	[residual_noise]=getResidualNoise(immagine); % Applicazione del filtro di denoising
	...
	count=0;
        ...
        fprintf('Correlazioni RN(%s) con i RPs: ', nomeFile);
        for i=1:(getNumCameras(pathReferencePatterns)+2);
	    ...
	    fileRP=strcat(pathReferencePatterns, char(filesRP(i)));
            load (fileRP); % carico referencePattern delle stesse dimensioni;
